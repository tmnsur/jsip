 ...
 * Response messages. The Response event encapsulates the Response message that
 * can be retrieved from {@link javax.sip.ResponseEvent#getResponse()}.
 ...
 * Response is correlated, i.e. the client transaction of the Request message
 * upon which this is a Response.
 * <p>
 ...
 * associated with.
 * <li>Response - the Response message received on the SipProvider that needs
 * passed to the application encapsulated in a ResponseEvent.
 * </ul>
 */
public class ResponseEvent extends EventObject {
	 ...
	 * @param clientTransaction - client transaction upon which this Response was
	 *                          sent
	...
	/**
	 * Gets the Dialog associated with the event or null if no dialog exists. This
	 * method separates transaction support from dialog support. This enables
	 * application developers to access the dialog associated to this event without
	 * having to query the transaction associated to the event. For example the
	 * transaction associated with the event may return 'null' because the final
	 * response for the transaction has already been received and the stack has no
	 * more record of the transaction. This situation can occur when a UAC sends
	 * requests out through a forking proxy. Responses that all refer to the same
	 * transaction may be sent by the targets of the fork but each response may be
	 * stamped with a different To tag, thus referring to different Dialogs on the
	 * UAC. The first final response terminates the transaction but the UAC may want
	 * to create a Dialog on a subsequent response.
	 *
