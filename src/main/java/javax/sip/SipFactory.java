 ...
 * <p>
 * For example an application can use the SipFactory to instantiate a NIST peer
 * SipStack object by setting the pathname to <code>gov.nist</code> and calling
 * the createSipStack method. The SipFactory would return a new instance of the
 * SipStack object at the following location:
 * <code>gov.nist.javax.sip.SipStackImpl.java</code> Because the space of domain
 * names is managed, this scheme ensures that collisions between two different
 * vendor's implementations will not happen. For example: a different vendor
 * with a domain name 'bea.com' would have their peer SipStack object located at
 * <code>com.bea.javax.sip.SipStackImpl.java</code>.
 * <p>
 ...
 * Implementation, therefore the <var>pathname</var> will have the initial value
 * of <code>gov.nist</code> for a new instance of the SipFactory. An application
 * must set the <var>pathname</var> of the SipFactory on retrieval of a new
 * instance of the factory in order to use a different vendors SIP stack from
 * that of the Reference Implementation. An application can not mix different
 * vendor's peer implementation objects.
 */
public class SipFactory {
	...
	// internal variable to ensure SipFactory only returns a single instance of the other Factories and SipStack
	...
	/**
	 * Returns an instance of a SipFactory. This is a singleton class so this method
	 * is the global access point for the SipFactory.
	 *
	...
	/**
	 * Creates an instance of a SipStack implementation based on the configuration
	 * properties object passed to this method. The recommended behavior is to not
	 * specify an "javax.sip.IP_ADDRESS" property in the Properties argument, in
	 * this case the "javax.sip.STACK_NAME" uniquely identifies the stack. A new
	 * stack instance will be returned for each different stack name associated with
	 * a specific vendor implementation. The ListeningPoint is used to configure the
	 * IP Address argument. For backwards compatibility, if a "javax.sip.IP_ADDRESS"
	 * is supplied, this method ensures that only one instance of a SipStack is
	 * returned to the application for that IP Address, independent of the number of
	 * times this method is called. Different SipStack instances are returned for
	 * each different IP address.
	 * <p>
	 * See {@link SipStack} for the expected format of the <code>properties</code>
	 * argument.
	 *
	...
	/**
	 * Creates an instance of the MessageFactory implementation. This method ensures
	 * that only one instance of a MessageFactory is returned to the application, no
	...
	/**
	 * Creates an instance of the HeaderFactory implementation. This method ensures
	 * that only one instance of a HeaderFactory is returned to the application, no
	 * matter how often this method is called.
	 *
	 * @throws PeerUnavailableException if peer class could not be found
	 */
	...
	/**
	 * Creates an instance of the AddressFactory implementation. This method ensures
	 * that only one instance of an AddressFactory is returned to the application,
	 * no matter how often this method is called.
	 *
	 * @throws PeerUnavailableException if peer class could not be found
	 */
	 ...
	 * vendor's implementation of this specification. The <var>pathname</var> must
	 * be the reverse domain name assigned to the vendor providing the
	 * implementation. An application must call {@link SipFactory#resetFactory()}
	 ...
	 * implementation of this specification as defined the naming convention. The
	 * pathname must be the reverse domain name assigned to the vendor providing
	 * this implementation. This value is defaulted to <code>gov.nist</code> the
	 ...
	 * application no longer holds references to them. This method must be called to
	 * reset the factories references to a specific vendors implementation of this
	 * specification before it creates another vendors implementation of this
	 * specification by changing the <var>pathname</var> of the SipFactory.
	 */
	...
	/**
	 * Private Utility method used by all create methods to return an instance of
	 * the supplied object.
	 */
	private Object createSipFactory(String objectClassName) throws PeerUnavailableException {
		...
		try {
			...
			String errmsg = "The Peer Factory: " + getPathName() + "." + objectClassName
	...
	private SipStack createStack(Properties properties) throws PeerUnavailableException {
		try {
			...
			Constructor sipStackConstructor = Class.forName(getPathName() + ".javax.sip.SipStackImpl")
			...
			String errmsg = "The Peer SIP Stack: " + getPathName() + ".javax.sip.SipStackImpl"
					+ " could not be instantiated. Ensure the Path Name has been set.";
			throw new PeerUnavailableException(errmsg, e);
		}
	}
	...
	/**
	 * Constructor for SipFactory class. This is private because applications are
	 * not permitted to create an instance of the SipFactory using "new".
	 */
	private SipFactory() {
