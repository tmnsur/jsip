 ...
 * <a href = "http://www.ietf.org/rfc/rfc3262.txt">RFC3262</a>, this header is
 * not part of RFC3261.
 * <p>
 ...
 * the RSeq in each subsequent reliable provisional response for the same
 * request MUST be greater by exactly one. RSeq numbers MUST NOT wrap around.
 * Because the initial one is chosen to be less than 2**31 - 1, but the maximum
 * is 2**32 - 1, there can be up to 2**31 reliable provisional responses per
...
public interface RSeqHeader extends Header {
	 ...
	 * The sequence number MUST be expressible as a 32-bit unsigned integer and MUST
	 * be less than 2**31.
	 *
