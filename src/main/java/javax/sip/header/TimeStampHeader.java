 ...
 * there is a delay in generating the response, the UAS SHOULD add a delay value
 * into the Timestamp value in the response. This value MUST contain the
 * difference between the time of sending of the response and receipt of the
 * request, measured in seconds. Although there is no normative behavior defined
 * here that makes use of the header, it allows for extensions or SIP
 * applications to obtain RTT estimates, that may be used to adjust the timeout
...
public interface TimeStampHeader extends Header {
	/**
	 * Sets the timestamp value of this TimeStampHeader to the new timestamp value
	...
	/**
	 * Sets the timestamp value of this TimeStampHeader to the new timestamp value
	 * passed to this method. This method allows applications to conveniantly use
	 * System.currentTimeMillis to set the timeStamp value.
	 * 
	 * @since v1.2
	 *
	...
	/**
	 * Sets the new delay value of the TimestampHeader to the delay parameter passed
	 * to this method
	 *
	 * @param delay - the new float delay value
	 * @throws InvalidArgumentException if the delay value argumenmt is a negative
