...
/**
 * This interface represents the Accept-Encoding request-header. A client
 * includes an AcceptEncodingHeader in a Request to tell the server what coding
 * schemes are acceptable in the Response e.g. compress, gzip.
 * <p>
 * If an AcceptEncodingHeader is present, and if the server cannot send a
 * Response which is acceptable according to the AcceptEncodingHeader, then the
...
public interface AcceptEncodingHeader extends Parameters, Encoding, Header {
	...
	/**
	 * Sets q-value for the encoding in this encoding value. Q-values allow the user
	 * to indicate the relative degree of preference for that encoding, using the
	 * qvalue scale from 0 to 1. If no q-value is present, the encoding should be
