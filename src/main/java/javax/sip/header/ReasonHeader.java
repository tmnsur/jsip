 ...
 * useful when creating services and also used to encapsulate a final status
 * code in a provisional response, which is needed to resolve the "Heterogeneous
 * Error Response Forking Problem".
 * <p>
 * The Reason header field appears to be most useful for BYE and CANCEL
 * requests, but it can appear in any request within a dialog, in any CANCEL
 * request and in 155 (Update Requested) responses. When used in requests,
 * clients and servers are free to ignore this header field. It has no impact on
 * protocol processing.
 * <p>
 ...
 * requests after receiving messages from a different protocol than SIP. In both
 * cases the client may be interested in knowing what triggered the SIP request.
...
public interface ReasonHeader extends Parameters, Header {
	...
	/**
	 * Sets the cause value of the ReasonHeader. Any SIP status code MAY appear in
	 * the Reason header field of a request, assuming the protocol field of the
	 * ReasonHeader is SIP.
	 *
