 ...
 * softphone's host name or IP address. The combination of the To tag, From tag,
 * and Call-ID completely defines a peer-to-peer SIP relationship between two
 * users and is referred to as a dialog. It MUST be the same for all requests
 * and responses sent by either User Agent in a dialog. It SHOULD be the same in
 * each registration from a User Agent.
 * <p>
 * All SIP User Agents must have a means to guarantee that the Call-ID header
 * fields they produce will not be inadvertently generated by any other User
 * Agent. Note that when requests are retried after certain failure responses
 * that solicit an amendment to a request (for example, a challenge for
 * authentication), these retried requests are not considered new requests, and
...
public interface CallIdHeader extends Header {
	...
	/**
	 * Returns the Call-Id of CallIdHeader. The CallId parameter uniquely identifies
	 * a series of messages within a dialogue.
	 *
	...
	/**
	 * Compare this CallIdHeader for equality with another. This method overrides
	 * the equals method in javax.sip.Header. This method specifies object equality
	 * as outlined by <a href = "http://www.ietf.org/rfc/rfc3261.txt">RFC3261</a>.
	 * Call-IDs are case-sensitive and are simply compared byte-by-byte.
	 *
	 ...
	 *         representing the same CallIdHeader as this, <code>false</code>
	 *         otherwise.
	 */
