...
/**
 * This interface represents the Subscription State header, as defined by
 ...
 * <ul>
 * <li>active - If the SubscriptionState header value is "active", it means that
 * the subscription has been accepted and (in general) has been authorized. If
 * the header also contains an "expires" parameter, the subscriber SHOULD take
 * it as the authoritative subscription duration and adjust accordingly. The
 * "retry-after" and "reason" parameters have no semantics for "active".
 * <li>pending - If the SubscriptionState value is "pending", the subscription
 * has been received by the notifier, but there is insufficient policy
 * information to grant or deny the subscription yet. If the header also
 * contains an "expires" parameter, the subscriber SHOULD take it as the
 * authoritative subscription duration and adjust accordingly. No further action
 * is necessary on the part of the subscriber. The "retry-after" and "reason"
 * parameters have no semantics for "pending".
...
public interface SubscriptionStateHeader extends Parameters, Header {
	/**
	 * Sets the relative expires value of the SubscriptionStateHeader. The expires value MUST be greater than zero
	...
	/**
	 * Sets the retry after value of the SubscriptionStateHeader. The retry after
	 * value MUST be greater than zero and MUST be less than 2**31.
	 *
	 ...
	 *
	 * @param reasonCode - the new reason code string value of the
	...
	/**
	 * Reason Code: The subscription has been terminated, but the subscriber SHOULD
	 * retry immediately with a new subscription. One primary use of such a status
	 * code is to allow migration of subscriptions between nodes. The "retry-after"
	 * parameter has no semantics for "deactivated".
	 */
	public static final String DEACTIVATED = "deactivated";

	/**
	 * Reason Code: The subscription has been terminated, but the client SHOULD
	 * retry at some later time. If a "retry-after" parameter is also present, the
	 * client SHOULD wait at least the number of seconds specified by that parameter
	 * before attempting to re-subscribe.
	 */
	public static final String PROBATION = "probation";

	/**
	 * Reason Code: The subscription has been terminated due to change in
	 * authorization policy. Clients SHOULD NOT attempt to re-subscribe. The
	 * "retry-after" parameter has no semantics for "rejected".
	 */
	public static final String REJECTED = "rejected";

	/**
	 * Reason Code: The subscription has been terminated because it was not
	 * refreshed before it expired. Clients MAY re-subscribe immediately. The
	 * "retry-after" parameter has no semantics for "timeout".
	 */
	public static final String TIMEOUT = "timeout";

	/**
	 * Reason Code: The subscription has been terminated because the notifier could
	 * not obtain authorization in a timely fashion. If a "retry-after" parameter is
	...
	/**
	 * Reason Code: The subscription has been terminated because the resource state
	...
	/**
	 * State: The subscription has been terminated, if a reason code is present, the
	 * client should behave as described in the reason code.
	 */
