 ...
 * of request or response it is in. A Contact header field value can contain a
 * display name, a URI with URI parameters, and header parameters.
 * <p>
 * The Contact header field provides a SIP or SIPS URI that can be used to
 * contact that specific instance of the User Agent for subsequent requests. The
 * Contact header field MUST be present and contain exactly one SIP or SIPS URI
 * in any request that can result in the establishment of a dialog. For the
 ...
 * header field value contains the URI at which the User Agent would like to
 ...
 * server sending a Success Response to an INIVTE must insert a ContactHeader in
 * the Response indicating the SIP address under which it is reachable most
 ...
 * contact header.
 * <li>Ambiguous Header: - A contact header is optional in response to INVITE,
 * OPTIONS, REGISTER and BYE requests.
 * </ul>
 *
 * The ContactHeader defines the Contact parameters "q" and "expires". The
 * <code>q-value</code> value is used to prioritize addresses in a list of
 * contact addresses. The <code>expires</code> value suggests an expiration
 * interval that indicates how long the client would like a registration to be
 * valid for a specific address. These parameters are only used when the Contact
 * is present in a:
 * <ul>
 * <li>REGISTER request
...
public interface ContactHeader extends HeaderAddress, Parameters, Header {
	...
	/**
	 * Sets the value of the <code>expires</code> parameter as delta-seconds. When a
	 * client sends a REGISTER request, it MAY suggest an expiration interval that
	 * indicates how long the client would like the registration to be valid for a
	 * specific address. There are two ways in which a client can suggest an
	 * expiration interval for a binding:
	 * <ul>
	 ...
	 * when more than one binding is given in a single REGISTER request, whereas the
	 * former suggests an expiration interval for all Contact header field values
	 * that do not contain the "expires" parameter. If neither mechanism for
	 * expressing a suggested expiration time is present in a REGISTER, the client
	 * is indicating its desire for the server to choose.
	 * <p>
	 * A User Agent requests the immediate removal of a binding by specifying an
	 * expiration interval of "0" for that contact address in a REGISTER request.
	 * User Agents SHOULD support this mechanism so that bindings can be removed
	 * before their expiration interval has passed. The REGISTER-specific Contact
	 * header field value of "*" applies to all registrations, but it MUST NOT be
	 * used unless the Expires header field is present with a value of "0". The "*"
	 * value can be determined if "this.getNameAddress().isWildcard() = = true".
	 *
	 * @param expires new relative value of the expires parameter. 0 implies removal
	 *                of Registration specified in Contact Header.
	...
	/**
	 * Returns the value of the <code>expires</code> parameter or -1 if no expires
	 * parameter was specified or if the parameter value cannot be parsed as an int.
	 *
	 ...
	 * A value of <code>-1</code> indicates the <code>qValue</code> parameter
	 * is not set.
	 *
	 ...
	 * preference amongst a set of locations. <code>q-values</code> are decimal
	...
	/**
	 * Returns a boolean value that indicates if the contact header has the format
	 * of Contact: *.
	 * 
