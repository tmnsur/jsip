 ...
 * hop has been selected. When the UAC creates a request, it MUST insert a Via
 * into that request. The protocol name and protocol version in the header field
 * MUST be SIP and 2.0, respectively. The Via header field value MUST contain a
 * branch parameter. This parameter is used to identify the transaction created
 * by that request. This parameter is used by both the client and the server.
 * <p>
 * <b>Branch Parameter:<br>
 * </b> The branch parameter value MUST be unique across space and time for all
 * requests sent by the User Agent. The exceptions to this rule are CANCEL and
 * ACK for non-2xx responses. A CANCEL request will have the same value of the
 * branch parameter as the request it cancels. An ACK for a non-2xx response
 * will also have the same branch ID as the INVITE whose response it
 * acknowledges.
 * <p>
 ...
 * <p>
 * A common way to create the branch value is to compute a cryptographic hash of
 * the To tag, From tag, Call-ID header field, the Request-URI of the request
 * received (before translation), the topmost Via header, and the sequence
 * number from the CSeq header field, in addition to any Proxy-Require and
 * Proxy-Authorization header fields that may be present. The algorithm used to
 * compute the hash is implementation-dependent.
 * <p>
 * <b>Via Processing Rules</b>
 * <ul>
 * <li>Generating Requests (UAC): The client originating the Request must insert
 * into the Request a ViaHeader containing its host name or network address and,
 * if not the default port number, the port number at which it wishes to receive
 * Responses. (Note that this port number can differ from the UDP source port
 ...
 * value into the copy before the existing Via header field values. This implies
 * that the proxy will compute its own branch parameter, which will be globally
 * unique for that branch, and contain the requisite magic cookie. Note that
 * this implies that the branch parameter will be different for different
 ...
 * a Request to a multicast group which already appears in any of the
 * ViaHeaders. This prevents a malfunctioning proxy server from causing loops.
 * Also, it cannot be guaranteed that a proxy server can always detect that the
 * address returned by a location service refers to a host listed in the
 * ViaHeader list, as a single host may have aliases or several network
 * interfaces.
 ...
 * client. Otherwise, the processing depends on whether the ViaHeader contains a
 * maddr parameter or is a receiver-tagged field.
 * <li>If the second ViaHeader contains a maddr parameter, send the Response to
 * the multicast address listed there, using the port indicated in "sent-by", or
 * port 5060 if none is present. The Response should be sent using the TTL
 ...
 * parameter, using the port indicated in the port value, or using port 5060 if
 * none is present.
 ...
 * the packet, send the Response to the actual packet source address, similar to
 * the treatment for receiver-tagged ViaHeaders.
 ...
 * protocol used to send the message, the client's host name or network address,
 * and possibly the port number at which it wishes to receive responses.
 * Transport protocols defined here are "UDP", "TCP", "TLS", and "SCTP". "TLS"
 * means TLS over TCP. When a request is sent to a SIPS URI, the protocol still
 * indicates "SIP", and the transport protocol is TLS. A Via header field value
 * can also contain parameters such as "maddr", "ttl", "received", and "branch".
 * A proxy sending a Request to a multicast address must add the maddr parameter
 * to its ViaHeader, and should add the ttl parameter. If a server receives a
 * Request which contained an maddr parameter in the topmost ViaHeader, it
 * should send the Response to the multicast address listed in the maddr
 * parameter. The received parameter is added only for receiver-added
 * ViaHeaders.
 * <p>
...
public interface ViaHeader extends Parameters, Header {
	 ...
	 * @throws InvalidArgumentException when the port value is not -1 and <1 or
	 *                                  >65535
	 */
	...
	/**
	 * Sets the value of the transport. This parameter specifies which transport
	 * protocol to use for sending requests and responses to this entity. The
	 * following values are defined: "udp", "tcp", "sctp", "tls", but other values
	 * may be used also.
	 *
	...
	/**
	 * Sets the value of the protocol parameter. This parameter specifies which
	 * protocol is used, for example "SIP/2.0".
	 *
	 ...
	 *                                  greater than 255, excluding -1 the default
	 *                                  not set value.
	 */
	public void setTTL(int ttl) throws InvalidArgumentException;

	/**
	 * Returns the value of the <code>maddr</code> parameter, or null if this is not
	 * set.
	 *
	 ...
	 * maddr parameter indicates the server address to be contacted for this user,
	 * overriding any address derived from the host field.
	 *
	...
	/**
	 * Gets the received paramater of the ViaHeader. Returns null if received does
	 * not exist.
	 *
	...
	/**
	 * Gets the branch paramater of the ViaHeader. Returns null if branch does not
	 * exist.
	 *
	...
	/**
	 * Sets the branch parameter of the ViaHeader to the newly supplied branch
	 * value. Note that when sending a Request within a transaction, branch id
	 * management will be the responsibility of the SipProvider; that is the
	 * application should not set this value. This method should only be used by the
	 * application when sending Requests outside of a transaction.
	 *
	...
	/**
	 * Set the rport part of this ViaHeader. This method indicates to the remote
	 * party that you want it to use rport. It is the applications responsisbility
	 * to call this method to inform the implementation to set the value of the
	 * rport. This allows a client to request that the server send the response back
	 * to the source IP address and port from which the request originated. See
	 * <a href = "http://www.ietf.org/rfc/rfc3581.txt">RFC3581</a>
	 *
	 *
	 ...
	 *
	 * @return the integer value of the rport or -1 if the rport parameter is not
	 *         set.
	 * @since v1.2
	 */
	public int getRPort();
	...
	/**
	 * Compare this ViaHeader for equality with another. This method overrides the
	 * equals method in javax.sip.Header. This method specifies object equality as
	 * outlined by <a href = "http://www.ietf.org/rfc/rfc3261.txt">RFC3261</a>. Two
	 * Via header fields are equal if their sent-protocol and sent-by fields are
	 * equal, both have the same set of parameters, and the values of all parameters
	 * are equal. When comparing header fields, field names are always
	 * case-insensitive. Unless otherwise stated in the definition of a particular
	 ...
	 * otherwise, values expressed as quoted strings are case-sensitive.
	 *
	 * @param obj the object to compare this ViaHeader with.
	 * @return <code>true</code> if <code>obj</code> is an instance of this class
	 *         representing the same ViaHeader as this, <code>false</code>
	 *         otherwise.
	 */
