 ...
 * <code>sips:</code> scheme. All SIP implementations MUST support the sip: URI
 * scheme.
 * <p>
 * SIP and SIPS URIs are used for addressing. They are similar to email
 * addresses in that they are of the form <code>user@host</code> where user is
 * either a user name or telephone number, and host is a host or domain name, or
 * a numeric IP address. Additionally, SIP and SIPS URIs may contain parameters
 * and headers (although headers are not legal in all contexts). A SipURI can be
 * embedded in web pages, business cards or other hyperlinks to indicate that a
 * particular user or service can be called via SIP.
 * <p>
 ...
 * <ul>
 * <li>User - The set of valid telephone-subscriber strings is a subset of valid
 * user strings. The user URI parameter exists to distinguish telephone numbers
 * from user names that happen to look like telephone numbers. If the user
 * string contains a telephone number formatted as a telephone-subscriber, the
 * user parameter value "phone" SHOULD be present. Even without this parameter,
 * recipients of SIP and SIPS URIs MAY interpret the pre-@ part as a telephone
 * number if local restrictions on the name space for user name allow it.
 ...
 * URIs) has proven to be a security risk in almost every case where it has been
 * used. For instance, transporting a PIN number in this field exposes the PIN.
 ...
 * any given parameter-name MUST NOT appear more than once. The SipURI interface
 * also provides convenience methods for the manipulation of popular parameters
 * in a SipURI address, namely:
 * <ul>
 ...
 * mechanism within a URI. The header names and values are encoded in ampersand
 * separated 'hname = hvalue' pairs. The special hname "body" indicates that the
 ...
 * </ul>
 * See section 19.1.2 of
 * <a href = "http://www.ietf.org/rfc/rfc3261.txt">RFC3261</a> for the use of
 * SIP and SIPS URI components based on the context in which the URI appears.
 *
...
public interface SipURI extends URI, Parameters {
	 ...
	 * being addressed. The user and the user password including the '@' sign make
	 * up the user-info.
	 *
	 ...
	 * because the passing of authentication information in clear text (such as
	 * URIs) has proven to be a security risk in almost every case where it has been
	 * used. The user password and the user including the @ sign make up the
	 * user-info.
	 *
	...
	/**
	 * Removes the port part of this SipURI. If no port is specified the stack will
...
// header manipulation methods
	 ...
	 * sip:jimmy@jcp.org?priority=urgent has a header "priority" whose value is
	 * "urgent".
	 *
	 ...
	 * @throws ParseException which signals that an error has been reached
	 *                        unexpectedly while parsing the name or value
	 *                        parameters.
	 */
	public void setHeader(String name, String value) throws ParseException;

	/**
	 * Returns an Iterator over the String names of all headers present in this
	 * SipURI.
	 *
	 ...
	 * The following values are defined: "udp", "tcp", "sctp", "tls", but other
	 * values may be used also. This method is equivalent to
