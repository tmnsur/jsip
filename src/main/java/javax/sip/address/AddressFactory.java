...
public interface AddressFactory {
	/**
	 * Creates a URI based on given URI string. The URI string is parsed in order to
	 * create the new URI instance. Depending on the scheme the returned may or may
	 * not be a SipURI or TelURL cast as a URI.
	 *
	 ...
	 * <li>The scheme followed by a colon character ('sip:') is appended to the
	 * result.
	 * <li>The user and host are then appended. Any character that is not a legal
	 * URI character is quoted.
	 * </ul>
	 * <br>
	 * The resulting URI string is then parsed in order to create the new SipURI
	 * instance as if by invoking the createURI(String) constructor; this may cause
	 * a URISyntaxException to be thrown.
	 * <p>
	 * An application that wishes to create a 'sips' URI should call the
	 * {@link SipURI#setSecure(boolean)} with an argument of 'true' on the returned
	 * SipURI.
	 *
	...
	/**
	 * Creates a TelURL based on given URI string. The scheme should not be included
	 * in the phoneNumber string argument.
	 *
	...
	/**
	 * Creates an Address with the new address string value. The address string is
	 * parsed in order to create the new Address instance. Valid arguments obey the
	 * syntax for <code>name-addr</code> tokens in RFC3261, for example "Bob
	 * <sip:bob@biloxi.com>". It is recommended to use the to use the name-addr form
	 * containing '<' '>' to avoid confusion of URI parameters. As a special case,
	 * the string argument "*" creates a wildcard Address object with the property
	 * that <code>((SipURI)Address.getURI()).getUser()</code> returns a String
	 ...
	 *
	 * @param displayName - the new string value of the display name of the address.
	 *                    A <code>null</code> value does not set the display name.
