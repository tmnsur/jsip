 ...
 *
 * <p>
 * For backwards compatibility reasons, the default behavior of the stack is to
 * consult the application provided Router implementation for all requests
 * outside of a dialog. This is controlled through the stack property
 * <code>javax.sip.USE_ROUTER_FOR_ALL_URIS</code> which defaults to
 * <code>true</code> when not set.
 *
 * <p>
 * This specification recommends to set the stack property
 * <code>javax.sip.USE_ROUTER_FOR_ALL_URIS</code> to <code>false</code>. This
 * will cause the stack to only consult the application provided Router
 * implementation for requests with a non-SIP URI as request URI (such as tel:
 * or pres:) and without Route headers. This enables an application to implement
 * DNS lookups and other resolution algorithms
 *
 * <p>
 ...
 * process if the topmost Route header contains no lr parameter(See Note below))
 * <li>Else, if the property <code>javax.sip.OUTBOUND_PROXY</code> is set, use
 * its value as the next hop
 ...
 *
 * <p>
 * <b>Note:</b> In case the topmost Route header contains no 'lr' parameter
 ...
 * of the SIP Stack object. The Router object must accept a SipStack as an
 * argument to the constructor in order for the Router to access attributes of
 * the SipStack The constructor of an object implementing the Router interface
 * must be <code>RouterImpl(SipStack sipStack, String outboundProxy) {}</code>
 * <p>
 * The routing policy can not be changed dynamically, i.e. the SipStack needs to
 * be deleted and re-created. Outbound proxy should be passed to the
 * {@link javax.sip.SipFactory#createSipStack(Properties)} method upon creation
 ...
 *
 * <p>
 * A UAC application which desires to use a particular outbound proxy should
 * prepend a Route header with the URI of that proxy (and 'lr' flag if
 * appropriate). Alternatively, it may achieve the same result by setting the
 * OUTBOUND_PROXY property (although the Route header approach is more flexible
...
public interface Router {
	/**
	 * Gets the Outbound Proxy parameter of this Router, this method may return null
	 * if no outbound proxy is defined.
	 *
	...
	/**
	 * Gets the next Hop from this Router for the specified request, this method may
	 * return <code>null</code> if a default route is not defined.
	 *
