...
/**
 * Implements a homogeneous consistent linked list. All the objects in the linked
...
public abstract class GenericObjectList extends LinkedList<GenericObject> implements Serializable, Cloneable {
	private static final Logger logger = Logger.getLogger(GenericObjectList.class.getName());
	...
	public boolean match(Object other) {
		...
		outer: while(hisIterator.hasNext()) {
			GenericObject hisobj = hisIterator.next();
			GenericObject myobj = null;
			ListIterator<GenericObject> myIterator = this.listIterator();
			while(myIterator.hasNext()) {
				myobj = myIterator.next();

				if(logger.isLoggable(Level.FINEST)) {
					logger.log(Level.FINEST, "Trying to match = {0}", myobj.encode());
				}

				if(GenericObject.isMySubclass(myobj.getClass()) && myobj.match(hisobj)
						|| GenericObjectList.isMySubclass(myobj.getClass()) && myobj.match(hisobj)) {
					break outer;
				}
			}

			if(logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST, "hisobj.encode(): {0}", hisobj.encode());
			}
