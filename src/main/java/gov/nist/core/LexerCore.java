...
public class LexerCore extends StringTokenizer {
	private static final Logger logger = Logger.getLogger(LexerCore.class.getName());
	...
	static {
		globalSymbolTable = new ConcurrentHashMap<>();
		lexerTables = new ConcurrentHashMap<>();
	}
	...
	public String lookupToken(int value) {
		if(value > START) {
			return globalSymbolTable.get(Integer.valueOf(value));
		}
		...
		for(int i = 0; i < ntokens; i++) {
			...
			if(startsId()) {
				...
				if(currentLexer.containsKey(idUppercase)) {
					tok.tokenType = currentLexer.get(idUppercase).intValue();
				} else {
					tok.tokenType = ID;
				}
			} else {
	...
	public Token match(int tok) throws ParseException {
		logger.log(Level.FINEST, "match {0}", tok);

		if(tok > START && tok < END) {
			if(tok == ID) {
				// Generic ID sought.
				if(!startsId()) {
					throw new ParseException(new String(buffer) + "\nID expected", ptr);
				}
	...
	public String comment() throws ParseException {
		...
		while(true) {
			...
			if(next == '\0') {
				throw new ParseException(new String(this.buffer) + " :unexpected EOL", this.ptr);
			}
	...
	/**
	 * Looks ahead in the inputBuffer for n chars and returns them as a string. Does not consume the input.
	 */
	...
	public String number() throws ParseException {
		...
		try {
			if(!isDigit(lookAhead(0))) {
				throw new ParseException(new String(buffer) + ": Unexpected token at " + lookAhead(0), ptr);
			}
