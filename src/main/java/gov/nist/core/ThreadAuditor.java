...
public class ThreadAuditor {
	...
	// moved to concurrent to avoid blocking witnessed by profiler
	...
	public class ThreadHandle {
		...
		public String toString() {
			StringBuilder toString = new StringBuilder().append("Thread Name: ").append(thread.getName())
	...
	public String auditThreads() {
		...
		while(it.hasNext()) {
			...
			if(!threadHandle.isThreadActive()) {
				...
				if(0 == auditReport.length()) {
					auditReport.append("Thread Auditor Report:\n");
				}

				auditReport.append("   Thread [" + thread.getName() + "] has failed to respond to an audit request.\n");
			}
	...
	public synchronized String toString() {
		...
		result.append("Thread Auditor - List of monitored threads:\n");
