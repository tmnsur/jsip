 ...
 * 
 * In addition to these, the following are meaningful properties for the NIST
 * SIP stack (specify these in the property array when you create the JAIN-SIP
 * statck):
 * <ul>
 * 
 ...
 * The server log accumulates the signaling trace.
 ...
 * strings DEBUG or INFO for level 32 and 16 respectively. If the value of this
 * property is set to LOG4J, then the effective log levels are determined from
 * the log4j settings file (e.g. log4j.properties). The logger name for the
 * stack is specified using the gov.nist.javax.sip.LOG4J_LOGGER_NAME property.
 * By default log4j logger name for the stack is the same as the stack name. For
 * example, <code>
 ...
 * TCP.</li>
 * 
 * <li><b>gov.nist.javax.sip.LOG_STACK_TRACE_ON_MESSAGE_SEND = true|false
 * </b><br/>
 ...
 * flag is set to true by default. When set to <it>false</it> the following
 * behaviors are enabled:
 * <ul>
 * 
 * <li>Turn off Merged requests Loop Detection:<br/>
 * The following behavior is turned off: If the request has no tag in the To
 ...
 * SHOULD generate a 482 (Loop Detected) response and pass it to the server
 * transaction.
 * 
 * </ul>
 * 
 * <li><b>gov.nist.javax.sip.IS_BACK_TO_BACK_USER_AGENT = [true|false] </b>
 * <br/>
 ...
 * 
 * <li><b>gov.nist.javax.sip.DELIVER_TERMINATED_EVENT_FOR_NULL_DIALOG =
 * [true|false] </b><br/>
 * If set to false (the default), the application does NOT get notified when a
 * Dialog in the NULL state is terminated. ( Dialogs in the NULL state are not
 * associated with an actual SIP Dialog. They are a programming convenience. A
 * Dialog is in the NULL state before the first response for the Dialog forming
 * Transaction). If set to true, the SipListener will get a
 * DialogTerminatedEvent when a Dialog in the NULL state is terminated.</li>
 * 
 ...
 * delivery. Default time is "infinity" - i.e. if the listener never responds,
 * the stack will hang on to a reference for the transaction and result in a
 * unusable thread stuck waiting for the lock to be released. A good value for
 * this property is the lifespan of the transaction or the expected blocking
 * delay in the listener.
 * 
 ...
 * example 3 minutes could be a good default Max time (seconds) an INVITE
 * transaction is supposed to live in the stack. This is to avoid any leaks in
 * whatever state the transaction can be in even if the application misbehaved
 * When the max time is reached, a timeout event will fire up to the application
 * listener so that the application can take action and then will be removed
 * from the stack after the typical lingering period of 8s in the stack
 * 
 ...
 * a good default Max time (seconds) a non INVITE transaction is supposed to
 * live in the stack. This is to avoid any leaks in whatever state the
 * transaction can be in even if the application misbehaved When the max time is
 * reached, a timeout event will fire up to the application listener so that the
 * application can take action and then will be removed from the stack after the
 * typical lingering period of 8s in the stack. There is a specific property as
 * a non INVITE property is short live as compared to INVITE and so can be
 * collected ore eagerly to save up on memory usage
 * 
 ...
 * the last IO operation on that socket. This allows to clean up after high
 * initial load of new calls that hang up or stay idle. Note that disconnecting
 * the socket does't end the SIP call. A new socket will be established when
 * needed for any existing calls by the SIP RFC spec.</li>
 * 
 ...
 * direct buffers. As Direct buffers reside outside of the heap memory, they can
 * lead to unforeseen out of memory exceptions as seen in
 * http://java.net/jira/browse/JSIP-430. This flag allows to use non direct
 ...
 * 
 * <li><b>gov.nist.javax.sip.CONGESTION_CONTROL_TIMEOUT = int </b> How much time
 * messages are allowed to wait in queue before being dropped due to stack being
 * too slow to respond. Default value is 8000 ms. The value is in milliseconds
 * </li>
 * 
 ...
 * their own TCP sockets. Sometimes however SIP devices are allowed to tunnel
 * multiple calls over a single socket. This can also be simulated with SIPP by
 * running "sipp -t t1".
 * 
 * In the stack each TCP socket has it's own thread. When all calls are using
 * the same socket they all use a single thread, which leads to severe
 * performance penalty, especially on multi-core machines.
 * 
 * This option instructs the SIP stack to use a thread pool and split the CPU
 * load between many threads. The number of the threads is specified in this
 * parameter.
 * 
 * The processing is split immediately after the parsing of the message. It
 * cannot be split before the parsing because in TCP the SIP message size is in
 * the Content-Length header of the message and the access to the TCP network
 * stream has to be synchronized. Additionally in TCP the message size can be
 * larger. This causes most of the parsing for all calls to occur in a single
 * thread, which may have impact on the performance in trivial applications
 * using a single socket for all calls. In most applications it doesn't have
 * performance impact.
 * 
 * If the phones/clients use separate TCP sockets for each call this option
 * doesn't have much impact, except the slightly increased memory footprint
 * caused by the thread pool. It is recommended to disable this option in this
 * case by setting it 0 or not setting it at all. You can simulate multi-socket
 * mode with "sipp -t t0".
 * 
 * With this option also we avoid closing the TCP socket when something fails,
 * because we must keep processing other messages for other calls.
 * 
 * Note: This option relies on accurate Content-Length headers in the SIP
 * messages. It cannot recover once a malformed message is processed, because
 * the stream iterator will not be aligned any more. Eventually the connection
 * will be closed.</li>
 * 
 * <li><b>gov.nist.javax.sip.DELIVER_UNSOLICITED_NOTIFY = [true|false] </b>
 * <br/>
 ...
 * 
 * <li><b>gov.nist.javax.sip.MAX_FORK_TIME_SECONDS = integer </b> Maximum time
 * for which the original transaction for which a forked response is received is
 * tracked. This property is only relevant to Dialog Stateful applications (
 * User Agents or B2BUA). When a forked response is received in this time
 * interval from when the original INVITE client transaction was sent, the stack
 * will place the original INVITE client transction in the ResponseEventExt and
 * deliver that to the application. The event handler can get the original
 * transaction from this event.</li>
 * 
 * <li><b>gov.nist.javax.sip.EARLY_DIALOG_TIMEOUT_SECONDS=integer </b> Maximum
 * time for which a dialog can remain in early state. This is defaulted to 3
 * minutes ( 180 seconds).</li>
 * 
 * <li><b>gov.nist.javax.sip.THREAD_PRIORITY=integer </b> Control the priority
 * of the threads started by the stack.</li>
 * 
 * <li><b>gov.nist.javax.sip.MESSAGE_PARSER_FACTORY = name of the class
 * implementing gov.nist.javax.sip.parser.MessageParserFactory</b> This factory
 * allows pluggable implementations of the MessageParser that will take care of
 * parsing the incoming messages. By example one could plug a lazy parser
 * through this factory.</li>
 * 
 * <li><b>gov.nist.javax.sip.MESSAGE_PROCESSOR_FACTORY = name of the class
 * implementing gov.nist.javax.sip.parser.MessageProcessorFactory</b> This
 * factory allows pluggable implementations of the MessageProcessor that will
 * take care of incoming messages. By example one could plug a NIO Processor
 * through this factory.</li>
 * 
 * <li><b>gov.nist.javax.sip.TIMER_CLASS_NAME = name of the class implementing
 * gov.nist.javax.sip.stack.timers.SipTimer</b> interface This allows pluggable
 * implementations of the Timer that will take care of scheduling the various
 * SIP Timers. By example one could plug a regular timer, a scheduled thread
 * pool executor.</li>
 * 
 * <li><b>gov.nist.javax.sip.DELIVER_RETRANSMITTED_ACK_TO_LISTENER=boolean</b> A
 * testing property that allows application to see the ACK for retransmitted 200
 * OK requests. <b>Note that this is for test purposes only</b></li>
 * 
 * *
 * <li><b>gov.nist.javax.sip.AGGRESSIVE_CLEANUP=boolean</b> A property that will
 * cleanup Dialog, and Transaction structures agrressively to improve memroy
 * usage and performance (up to 50% gain). However one needs to be careful in
 * its code on how and when it accesses transaction and dialog data since it
 * cleans up aggressively when transactions changes state to COMPLETED or
 * TERMINATED and for Dialog once the ACK is received/sent</li>
 * 
 * <li><b>gov.nist.javax.sip.MIN_KEEPALIVE_TIME_SECONDS = integer</b> Minimum
 * time between keep alive pings (CRLF CRLF) from clients. If pings arrive with
 * less than this frequency they will be replied with CRLF CRLF if greater they
 * will be rejected. The default is -1 (i.e. do not respond to CRLF CRLF).</li>
 * 
 * <li><b>gov.nist.javax.sip.DIALOG_TIMEOUT_FACTOR= integer</b> Default to 64.
 * The number of ticks before a dialog that does not receive an ACK receives a
 * Timeout notification. Note that this is only relevant if the registered
 * SipListener is of type SipListenerExt</li>
 * 
 * <li><b>gov.nist.javax.sip.SIP_MESSAGE_VALVE= String</b> Default to null. The
 * class name of your custom valve component. An instance of this class will be
 * created and the SIPMessageValve.processRequest/Response() methods will be
 * called for every message before any long-lived SIP Stack resources are
 * allocated (no transactions, no dialogs). From within the processRequest
 * callback implementation you can drop messages, send a response statelessly or
 * otherwise transform/pre-process the message before it reaches the next steps
 * of the pipeline. Similarly from processResponse() you can manipulate a
 * response or drop it silently, but dropping responses is not recommended,
 * because the transaction already exists when the request for the response was
 * sent.</li>
 * 
 * <li><b>gov.nist.javax.sip.SIP_EVENT_INTERCEPTOR</b> Default to null. The
 * class name of your custom interceptor object. An instance of this object will
 * be created at initialization of the stack. You must implement the interface
 * gov.nist.javax.sip.stack.SIPEventInterceptor and handle the lifecycle
 * callbacks. This interface is the solution for
 * https://jain-sip.dev.java.net/issues/show_bug.cgi?id=337 . It allows to wrap
 * the JSIP pipeline and execute custom analysis logic as SIP messages advance
 * through the pipeline checkpoints. One example implementation of this
 * interceptor is <b>gov.nist.javax.sip.stack.CallAnalysisInterceptor</b>, which
 * will periodically check for requests stuck in the JAIN SIP threads and if
 * some request is taking too long it will log the stack traces for all threads.
 * The logging can occur only on certain events, so it will not overwhelm the
 * CPU. The overall performance penalty by using this class in production under
 * load is only 2% peak on average laptop machine. There is minimal locking
 * inside. One known limitation of this feature is that you must use
 * gov.nist.javax.sip.REENTRANT_LISTENER=true to ensure that the request will be
 * processed in the original thread completely for UDP.</li>
 * 
 * <li><b>gov.nist.javax.sip.TLS_CLIENT_PROTOCOLS = String </b> Comma-separated
 * list of protocols to use when creating outgoing TLS connections. The default
 * is "TLSv1.2, TLSv1.1, TLSv1". It's advisable not to use SSL protocols because
 * of
 * http://googleonlinesecurity.blogspot.fr/2014/10/this-poodle-bites-exploiting-ssl-30.html
 * </li>
 * 
 * <li><b>gov.nist.javax.sip.TLS_SECURITY_POLICY = String </b> The fully
 * qualified path name of a TLS Security Policy implementation that is consulted
 * for certificate verification of outbund TLS connections.</li>
 * 
 * <li><b>gov.nist.javax.sip.TLS_CLIENT_AUTH_TYPE = String </b> Valid values are
 * Default (backward compatible with previous versions) , Enabled, Want,
 * Disabled or DisabledAll. Set to Enabled if you want the SSL stack to require
 * a valid certificate chain from the client before accepting a connection. Set
 * to Want if you want the SSL stack to request a client Certificate, but not
 * fail if one isn't presented. A Disabled value will not require a certificate
 * chain for the Server Connection. A DisabledAll will not require a certificate
 * chain for both Server and Client Connections.</li>
 *
 * <li><b>gov.nist.javax.sip.RELIABLE_CONNECTION_KEEP_ALIVE_TIMEOUT</b> Value in
 * seconds which is used as default keepalive timeout (See also
 * http://tools.ietf.org/html/rfc5626#section-4.4.1). Defaults to "infinity"
 * seconds (i.e. timeout event not delivered).</li>
 *
 * <li><b>gov.nist.javax.sip.TCP_NODELAY = [true|false]</b> Whether or not to
 * disable Nagle's algorithm for TCP sockets. Defaults to {@code false}.</li>
 * 
 * <li><b>gov.nist.javax.sip.SSL_HANDSHAKE_TIMEOUT</b> Value in seconds which is
 * used as default timeout for performing the SSL Handshake This prevents bad
 * clients of connecting without sending any data to block the server</li>
 * 
 * <li><b>gov.nist.javax.sip.SSL_RENEGOTIATION_ENABLED = [true|false]</b>
 * Default value is <b>true</b>. Allow or disallow SSL renegotiation to resolve
 * potential DoS problem - <a href=
 * "http://web.nvd.nist.gov/view/vuln/detail?vulnId=CVE-2011-1473">reference</a>
 * and <a href=
 * "http://www.ietf.org/mail-archive/web/tls/current/msg07553.html">another
 ...
 * </code> <br>
 * If not trustStorePassword is provided then the keyStorePassword will be used
 * for both. The trust store can be changed, to a separate file with the
...
public class SipStackImpl extends SIPTransactionStack implements javax.sip.SipStack, SipStackExt {
	private static final Logger logger = Logger.getLogger(SipStackImpl.class.getName());
	...
	private void reInitialize() {
		...
		if(!getTimer().isStarted()) {
			String defaultTimerName = configurationProperties.getProperty("gov.nist.javax.sip.TIMER_CLASS_NAME",
			...
			try {
				...
				logger.log(Level.SEVERE, "Bad configuration value for gov.nist.javax.sip.TIMER_CLASS_NAME", e);
			}
		}
	}
	 ...
	 * 
	 * @param configurationProperties -- stack configuration properties including
	 *                                NIST-specific extensions.
	...
	public SipStackImpl(Properties configurationProperties) throws PeerUnavailableException {
		...
		super.setReliableConnectionKeepAliveTimeout(1000 * Integer.parseInt(configurationProperties
				.getProperty("gov.nist.javax.sip.RELIABLE_CONNECTION_KEEP_ALIVE_TIMEOUT", "-1")));
		...
		super.setSslHandshakeTimeout(
				Long.parseLong(configurationProperties.getProperty("gov.nist.javax.sip.SSL_HANDSHAKE_TIMEOUT", "-1")));
		...
		ByteBufferFactory.getInstance().setUseDirect(Boolean.valueOf(configurationProperties
				.getProperty("gov.nist.javax.sip.stack.USE_DIRECT_BUFFERS", Boolean.TRUE.toString())));
		...
		if(extensionMethods != null) {
			...
			while(st.hasMoreTokens()) {
				...
				if(em.equalsIgnoreCase(Request.BYE) || em.equalsIgnoreCase(Request.INVITE)
						|| em.equalsIgnoreCase(Request.SUBSCRIBE) || em.equalsIgnoreCase(Request.NOTIFY)
						|| em.equalsIgnoreCase(Request.ACK) || em.equalsIgnoreCase(Request.OPTIONS)) {
		...
		if (clientAuthType != null) {
			...
			logger.log(Level.INFO, "using {0} tls auth policy", clientAuthType);
		}
		...
		if(keyStoreFile != null) {
			...
			String trustStorePassword = configurationProperties.getProperty("javax.net.ssl.trustStorePassword",
					keyStorePassword);
			...
			try {
				this.networkLayer = new SslNetworkLayer(this, trustStoreFile, keyStoreFile,
						keyStorePassword != null ? keyStorePassword.toCharArray() : null,
						trustStorePassword != null ? trustStorePassword.toCharArray() : null, keyStoreType,
						trustStoreType);
			} catch(Exception e1) {
				logger.log(Level.SEVERE, "could not instantiate SSL networking", e1);
			}
		}

		// Set the auto dialog support flag.
		super.isAutomaticDialogSupportEnabled = configurationProperties
				.getProperty("javax.sip.AUTOMATIC_DIALOG_SUPPORT", "on").equalsIgnoreCase("on");
		...
		super.isServerLoopDetectionEnabled = configurationProperties
				.getProperty("gov.nist.javax.sip.SERVER_LOOP_DETECTION", "on").equalsIgnoreCase("on");
		...
		if(configurationProperties.getProperty("gov.nist.javax.sip.MAX_LISTENER_RESPONSE_TIME") != null) {
			super.maxListenerResponseTime = Integer
					.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.MAX_LISTENER_RESPONSE_TIME"));
		...
		if(configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_INVITE") != null) {
			super.maxTxLifetimeInvite = Integer.parseInt(configurationProperties
					.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_INVITE"));

			if(super.getMaxTxLifetimeInvite() <= 0) {
				throw new PeerUnavailableException("Bad configuration parameter"
						+ " gov.nist.javax.sip.MAX_TX_LIFETIME_INVITE : should be positive");
			}
		} else {
		...
		if(configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_NON_INVITE") != null) {
			super.maxTxLifetimeNonInvite = Integer.parseInt(configurationProperties
					.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_NON_INVITE"));

			if(super.getMaxTxLifetimeNonInvite() <= 0) {
				throw new PeerUnavailableException("Bad configuration parameter"
						+ " gov.nist.javax.sip.MAX_TX_LIFETIME_NON_INVITE : should be positive");
			}
		} else {
			super.maxTxLifetimeNonInvite = -1;
		}

		setDeliverTerminatedEventForAck(configurationProperties
				.getProperty("gov.nist.javax.sip.DELIVER_TERMINATED_EVENT_FOR_ACK", FALSE).equalsIgnoreCase("true"));

		setDeliverUnsolicitedNotify(Boolean.parseBoolean(configurationProperties
				.getProperty("gov.nist.javax.sip.DELIVER_UNSOLICITED_NOTIFY", FALSE)));
		...
		if(tlsPolicyPath == null) {
			...
			logger.log(Level.WARNING, "using default tls security policy");
		}
		...
		/*
		 * gets the NetworkLayer implementation, if any. Note that this is a NIST only
		 * feature.
		 */
		...
		if(maxConnections != null) {
			try {
				...
				logger.log(Level.SEVERE, "max connections - bad value", ex);
			}
		}
		...
		if(threadPoolSize != null) {
			try {
				...
				logger.log(Level.SEVERE, "thread pool size - bad value", ex);
			}
		}

		int congetstionControlTimeout = Integer
				.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.CONGESTION_CONTROL_TIMEOUT", "8000"));
		...
		if (tcpTreadPoolSize != null) {
			try {
				int threads = Integer.parseInt(tcpTreadPoolSize);
				...
				logger.log(Level.SEVERE, "TCP post-parse thread pool size - bad value {0} : {1}", new Object[] {
						tcpTreadPoolSize, ex.getMessage()});
			}
		}
		...
		if(serverTransactionTableSize != null) {
			try {
				this.serverTransactionTableHighwaterMark = Integer.parseInt(serverTransactionTableSize);
				this.serverTransactionTableLowaterMark = this.serverTransactionTableHighwaterMark * 80 / 100;
				// Low water is 80% of high water
			} catch(NumberFormatException ex) {
				logger.log(Level.SEVERE, "transaction table size - bad value", ex);
			}
		} else {
			// Issue 256 : consistent with MAX_CLIENT_TRANSACTIONS, if the
			// MAX_SERVER_TRANSACTIONS is not set
		...
		if(clientTransactionTableSize != null) {
			try {
				this.clientTransactionTableHiwaterMark = Integer.parseInt(clientTransactionTableSize);
		 ...
		 */
		String securityManagerProviderKey = "gov.nist.javax.sip.SECURITY_MANAGER_PROVIDER";

		if(configurationProperties.containsKey(securityManagerProviderKey)) {
			String path = configurationProperties.getProperty(securityManagerProviderKey);
			...
			try {
				...
				throw new PeerUnavailableException("can't find or instantiate SecurityManagerProvider"
						+ " implementation: " + path, e);
			}
		} else {
		...
		if(readTimeout != null) {
			try {
				...
				if(rt >= 100) {
					...
					logger.log(Level.SEVERE, "Value too low {0}", readTimeout);
				}
			} catch(NumberFormatException nfe) {
				logger.log(Level.SEVERE, "Bad read timeout {0}", readTimeout);
			}
		}
		...
		if(stunAddr != null) {
			logger.log(Level.WARNING, "Ignoring obsolete property " + "gov.nist.javax.sip.STUN_SERVER");
		}

		String maxMsgSize = configurationProperties.getProperty("gov.nist.javax.sip.MAX_MESSAGE_SIZE");
		try {
			if(maxMsgSize != null) {
				super.maxMessageSize = Integer.parseInt(maxMsgSize);
			...
			logger.log(Level.SEVERE, "maxMessageSize - bad value", ex);
		}
		...
		if(interval != null) {
			try {
				...
				getThreadAuditor().setPingIntervalInMillisecs(Long.parseLong(interval) / 2);
			} catch(NumberFormatException ex) {
				logger.log(Level.SEVERE, ex,
						() -> MessageFormat.format("THREAD_AUDIT_INTERVAL_IN_MILLISECS - bad value [{0}]", interval));
			}
		}

		// added property for testing
		this.setNon2XXAckPassedToListener(Boolean.parseBoolean(
				configurationProperties.getProperty("gov.nist.javax.sip.PASS_INVITE_NON_2XX_ACK_TO_LISTENER", FALSE)));

		this.generateTimeStampHeader = Boolean.parseBoolean(configurationProperties
				.getProperty("gov.nist.javax.sip.AUTO_GENERATE_TIMESTAMP", FALSE));
		...
		if(messageLogFactoryClasspath != null) {
			try {
				...
				logger.log(Level.SEVERE, "Bad configuration value for LOG_FACTORY -- using default logger");
		...
		boolean computeContentLength = configurationProperties
				.getProperty("gov.nist.javax.sip.COMPUTE_CONTENT_LENGTH_FROM_MESSAGE_BODY", FALSE)
		...
		if (tlsClientProtocols != null) {
			/*
			 * http://java.net/jira/browse/JSIP-451 accepts protocol list enclosed in "" and separated by spaces
			 * and/or commas
			 */
			StringTokenizer st = new StringTokenizer(tlsClientProtocols, "\" ,");
			String[] protocols = new String[st.countTokens()];

			logger.log(Level.FINEST, "TLS Client Protocols");

			int i = 0;
			while(st.hasMoreTokens()) {
				protocols[i] = st.nextToken();

				logger.log(Level.FINEST, "TLS Client Protocol: {0}", protocols[i]);
		...
		super.rfc2543Supported = configurationProperties
				.getProperty("gov.nist.javax.sip.RFC_2543_SUPPORT_ENABLED", "true").equalsIgnoreCase("true");

		super.cancelClientTransactionChecked = configurationProperties
				.getProperty("gov.nist.javax.sip.CANCEL_CLIENT_TRANSACTION_CHECKED", "true").equalsIgnoreCase("true");
		super.logStackTraceOnMessageSend = configurationProperties
				.getProperty("gov.nist.javax.sip.LOG_STACK_TRACE_ON_MESSAGE_SEND", FALSE).equalsIgnoreCase("true");

		logger.log(Level.FINEST, "created Sip stack. Properties: {0}", configurationProperties);
		...
		if(in != null) {
			try {
				...
				logger.log(Level.SEVERE, "Could not open build timestamp.");
			}
		}

		String bufferSize = configurationProperties.getProperty("gov.nist.javax.sip.RECEIVE_UDP_BUFFER_SIZE",
				MAX_DATAGRAM_SIZE.toString());
		int bufferSizeInteger = Integer.parseInt(bufferSize);

		super.setReceiveUdpBufferSize(bufferSizeInteger);

		bufferSize = configurationProperties.getProperty("gov.nist.javax.sip.SEND_UDP_BUFFER_SIZE",
				MAX_DATAGRAM_SIZE.toString());
		bufferSizeInteger = Integer.parseInt(bufferSize);

		super.setSendUdpBufferSize(bufferSizeInteger);

		super.isBackToBackUserAgent = Boolean.parseBoolean(configurationProperties
				.getProperty("gov.nist.javax.sip.IS_BACK_TO_BACK_USER_AGENT", Boolean.FALSE.toString()));
		super.checkBranchId = Boolean.parseBoolean(configurationProperties
				.getProperty("gov.nist.javax.sip.REJECT_STRAY_RESPONSES", Boolean.FALSE.toString()));

		super.isDialogTerminatedEventDeliveredForNullDialog = (Boolean.parseBoolean(configurationProperties
				.getProperty("gov.nist.javax.sip.DELIVER_TERMINATED_EVENT_FOR_NULL_DIALOG", Boolean.FALSE.toString())));

		super.maxForkTime = Integer
				.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.MAX_FORK_TIME_SECONDS", "0"));
		...
		super.minKeepAliveInterval = Integer
				.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.MIN_KEEPALIVE_TIME_SECONDS", "-1"));

		super.deliverRetransmittedAckToListener = Boolean.parseBoolean(configurationProperties
				.getProperty("gov.nist.javax.sip.DELIVER_RETRANSMITTED_ACK_TO_LISTENER", FALSE));

		super.dialogTimeoutFactor = Integer
				.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.DIALOG_TIMEOUT_FACTOR", "64"));

		String messageParserFactoryName = configurationProperties
				.getProperty("gov.nist.javax.sip.MESSAGE_PARSER_FACTORY", StringMsgParserFactory.class.getName());
		try {
			super.messageParserFactory = (MessageParserFactory) Class.forName(messageParserFactoryName).newInstance();
		} catch (Exception e) {
			logger.log(Level.SEVERE, "Bad configuration value for gov.nist.javax.sip.MESSAGE_PARSER_FACTORY", e);
		}

		String messageProcessorFactoryName = configurationProperties.getProperty(
				"gov.nist.javax.sip.MESSAGE_PROCESSOR_FACTORY", OIOMessageProcessorFactory.class.getName());
		try {
			super.messageProcessorFactory = (MessageProcessorFactory) Class.forName(messageProcessorFactoryName)
					.newInstance();
		} catch (Exception e) {
			logger.log(Level.SEVERE, "Bad configuration value for gov.nist.javax.sip.MESSAGE_PROCESSOR_FACTORY", e);
		}

		String maxIdleTimeString = configurationProperties.getProperty("gov.nist.javax.sip.NIO_MAX_SOCKET_IDLE_TIME",
				"7200000");
		try {
			super.nioSocketMaxIdleTime = Long.parseLong(maxIdleTimeString);
		} catch(Exception e) {
			logger.log(Level.SEVERE, e, () -> MessageFormat.format("Bad configuration value for"
					+ " gov.nist.javax.sip.NIO_MAX_SOCKET_IDLE_TIME: {0}", maxIdleTimeString));
		}
		...
		super.aggressiveCleanup = Boolean.parseBoolean(configurationProperties
				.getProperty("gov.nist.javax.sip.AGGRESSIVE_CLEANUP", Boolean.FALSE.toString()));
		...
		if(valveClassName != null && !valveClassName.equals("")) {
			try {
				...
				try {
					...
					logger.log(Level.SEVERE, "Error intializing SIPMessageValve", e);
				}
			} catch(Exception e) {
				logger.log(Level.SEVERE, "Bad configuration value for gov.nist.javax.sip.SIP_MESSAGE_VALVE", e);
			}
		}

		String interceptorClassName = configurationProperties.getProperty("gov.nist.javax.sip.SIP_EVENT_INTERCEPTOR",
				null);
				...
				new Thread() {
					public void run() {
						try {
							...
							logger.log(Level.SEVERE, "Error intializing SIPEventInterceptor", e);
						}
					}
				}.start();
			} catch(Exception e) {
				logger.log(Level.SEVERE, "Bad configuration value for gov.nist.javax.sip.SIP_EVENT_INTERCEPTOR", e);
			}
		}

		setSslRenegotiationEnabled(Boolean.parseBoolean(
				configurationProperties.getProperty("gov.nist.javax.sip.SSL_RENEGOTIATION_ENABLED", "true")));
	}
	 ...
	 */
	@Override
	public synchronized ListeningPoint createListeningPoint(String address, int port, String transport)
			throws TransportNotSupportedException, InvalidArgumentException {
		logger.entering(SipStackImpl.class.getName(), "createListeningPoint",
				new Object[] {address, port, transport});
		...
		if(!transport.equalsIgnoreCase("UDP") && !transport.equalsIgnoreCase("TLS")
				...
				&& !transport.equalsIgnoreCase("WS") && !transport.equalsIgnoreCase("WSS")) {
		...
		try {
			...
			logger.log(Level.FINEST, "Created Message Processor: {0}, port: {1}, transport: {2}",
					new Object[] {address, port, transport});
			...
			if(socketTimeoutAuditor == null && nioSocketMaxIdleTime > 0
					&& messageProcessor instanceof ConnectionOrientedMessageProcessor) {
				// https://java.net/jira/browse/JSIP-471 use property from the stack instead of
				// hard coded 20s
			...
			logger.log(Level.SEVERE, "Invalid argument address: {0}, port: {1}, transport: {2}",
					new Object[] {address, port, transport});
	...
	public SipProvider createSipProvider(ListeningPoint listeningPoint) throws ObjectInUseException {
		...
		logger.log(Level.FINEST, "createSipProvider: {0}", listeningPoint);
	...
	/**
	 * Finalization -- stop the stack on finalization. Exit the transaction scanner
	 ...
	 */
	public void stop() {
		logger.log(Level.FINEST, "stopStack -- stoppping the stack");
		...
		/*
		 * Check for presence of an event scanner ( may happen if stack is stopped
		 * before listener is attached ).
		 */
	...
	/**
	 * Get the listener for the stack. A stack can have only one listener. To get an
	 * event from a provider, the listener has to be registered with the provider.
	 * The SipListener is application code.
	 * 
	...
	/**
	 * Get the TLS Security Policy implementation for the stack. The
	 * TlsSecurityPolicy is application code.
	 *
	 ...
	 * (non-Javadoc)
	 * 
	 * @see gov.nist.javax.sip.SipStackExt#getAuthenticationHelper(gov.nist.javax
	...
	/**
	 * Set the list of cipher suites supported by the stack. A stack can have only
	 * one set of suites. These are not validated against the supported cipher
	 * suites of the java runtime, so specifying a cipher here does not guarantee
	...
	/**
	 * Set the list of protocols supported by the stack for outgoing TLS
	 * connections. A stack can have only one set of protocols. These are not
	 * validated against the supported protocols of the java runtime, so specifying
	 * a protocol here does not guarantee that it will work.<br>
	 ...
	 * </ul>
	 * 
	 * <b>NOTE: This function must be called before creating a
	 * TLSMessageChannel.</b>
	 * 
	 * @param String [] The new set of protocols to use for outgoing TLS
	 *               connections.
	 * @return
	 * 
	 */
