...
public interface ParseExceptionListener {
	/**
	 * This gets called from the parser when a parse error is generated. The handler
	 * is supposed to introspect on the error class and header name to handle the
	 * error appropriately. The error can be handled by :
	 * <ul>
	 * <li>1. Re-throwing an exception and aborting the parse.
	 * <li>2. Ignoring the header (attach the unparseable header to the SIPMessage
	 * being parsed).
