...
/**
 * 
 * Decodes a web socket frame from wire protocol version 8 format. This code was originally based on
 * <a href="https://github.com/joewalnes/webbit">webbit</a>.
 */
public class WebSocketCodec {
	private static final Logger logger = Logger.getLogger(WebSocketCodec.class.getName());
	...
	public byte[] decode(InputStream is) throws Exception {
		do {
			...
			if(availToRead > bytesLeft - 1) {
				...
				logger.log(Level.FINEST, "Increasing buffer size from {0} avail {1} newSize {2}",
						new Object[] {decodeBuffer.length, availToRead, newSize});
		...
		/*
		 * Start over from scratch. If the frame is big this may be repeated a few times O(logN)
		...
		if(writeIndex < 4) {
			logger.log(Level.FINEST, "Abort decode. Write index is at {0}", writeIndex);
		...
		logger.log(Level.FINEST, "Decoding WebSocket Frame opCode: {0}", frameOpcode);
		...
		try {
			...
			logger.log(Level.FINEST, "Decoding WebSocket Frame length: {0}", framePayloadLength);
		...
		if(writeIndex < totalPacketLength) {
			logger.log(Level.FINEST, "Abort decode. Write index is at {0} and totalPacketLength is {1}",
					new Object[] {writeIndex, totalPacketLength});
		...
		logger.log(Level.FINEST, "writeIndex: {0} {1}", new Object[] { writeIndex, totalPacketLength });
	...
	protected static byte[] encode(byte[] msg, int rsv, boolean fin, boolean maskPayload, byte opcode)
			throws Exception {
		ByteArrayOutputStream frame = new ByteArrayOutputStream();

		long length = msg.length;

		logger.log(Level.FINEST, "Encoding WebSocket Frame opCode: {0}, length: {1}", new Object[] {opcode, length});
		...
		if(length <= 125) {
			frame.write(b0);
			frame.write((byte) (maskPayload ? 0x80 | (byte) length : (byte) length));
	...
	private void protocolChecks() {
		...
		if(frameOpcode > 7) {
			...
			// data frame check for reserved data frame opcodes
