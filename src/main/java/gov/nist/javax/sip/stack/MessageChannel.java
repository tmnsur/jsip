...
public abstract class MessageChannel {
	 ...
	 *
	 * @return a string contianing the ip address or host name of the sender of the
	 *         message.
	 */
	...
	/**
	 * Generate a key which identifies the message channel. This allows us to cache
	 * the message channel.
	 */
	...
	/**
	 * Send the message (after it has been formatted), to a specified address and a
	 * specified port
	 *
	 ...
	 */
	protected abstract void sendMessage(byte[] message, InetAddress receiverAddress, int receiverPort,
			boolean reconnectFlag) throws IOException;
	...
	public void sendMessage(final SIPMessage sipMessage, Hop hop) throws IOException {
		...
		try {
			for(MessageProcessor processor : getSIPStack().getMessageProcessors()) {
				if(processor.getIpAddress().equals(hopAddr) && processor.getPort() == hop.getPort()
						&& processor.getTransport().equalsIgnoreCase(hop.getTransport())) {
					MessageChannel messageChannel = processor.createMessageChannel(hopAddr, hop.getPort());
	...
	/**
	 * Convenience function to get the raw IP source address of a SIP message as a
	 * String.
	 */
	...
	public static String getKey(InetAddress inetAddr, int port, String transport) {
		// http://java.net/jira/browse/JSIP-413 Concurrency issue within
		// MessageChannel.java when using IPv6 addresses
	...
	/**
	 * Get the Via header for this transport. Note that this does not set a branch
	 * identifier.
	 *
	...
	/**
	 * Get the via header host:port structure. This is extracted from the topmost
	...
	public void logMessage(SIPMessage sipMessage, InetAddress address, int port, long time) {
		...
		logger.log(Level.FINEST, "sipMessage: {0}, host:port: {1}:{2}, remote host:port: {3}:{4} time: {5}",
				new Object[] { sipMessage, this.getHost(), this.getPort(), address.getHostAddress(), currentPort,
						time });
	}

	/**
	 * Log a response received at this message channel. This is used for processing
	 * incoming responses to a client transaction.
	 *
	...
	public void logResponse(SIPResponse sipResponse, long receptionTime, String status) {
		...
		logger.log(Level.FINEST, "sipResponse: {0}, from: {1}, to: {2}, status: {3}, receptionTime: {4}",
				new Object[] { sipResponse, getPeerAddress() + ":" + peerport, this.getHost() + ":" + getPort(),
						status, receptionTime });
	}
	...
	protected final String createBadReqRes(String badReq, ParseException pe) {
		...
		if (!(this instanceof UDPMessageChannel) || clength + buf.length() + ContentTypeHeader.NAME.length()
	 ...
	 * @return
	 *
	 *         Note: some limitations here: does not work for short forms of
	 *         headers, or continuations; problems when header names appear in other
