...
public class SIPServerTransactionImpl extends SIPTransactionImpl implements SIPServerTransaction {
	...
	private static final Logger logger = Logger.getLogger(SIPServerTransactionImpl.class.getName());
	...
	// needed because we nullify the dialog ref early and keep only the dialogId to save on mem and help GC
	...
	// we nullify the last response fast to save on mem and help GC, but we keep only the information needed
	...
	class ProvisionalResponseTask extends SIPStackTimerTask {
		...
		public void runTask() {
			 ...
			 * retransmission (T1 is defined in Section 17 of RFC 3261). Once passed to the
			 * server transaction, it is added to an internal list of unacknowledged
			 * reliable provisional responses. The transaction layer will forward each
			 * retransmission passed from the UAS core.
			 *
			 ...
			 * 2xx, but retransmissions of PRACK take place independently of reception of
			 * 1xx.
			 */
	 ...
	 * 200ms. if the TU does not do so.
	 */
	class SendTrying extends SIPStackTimerTask {
		protected SendTrying() {
			logger.log(Level.FINEST, "scheduled timer for {0}", SIPServerTransactionImpl.this);
		}
		...
		public void runTask() {
			...
			if(realState < 0 || TransactionState.TRYING_VALUE == realState) {
				logger.log(Level.FINEST, "sending Trying current state: {0}", serverTransaction.getRealState());

				try {
					serverTransaction.sendMessage(serverTransaction.getOriginalRequest().createResponse(100, "Trying"));

					logger.log(Level.FINEST, "trying sent {0}", serverTransaction.getRealState());
				} catch(IOException ex) {
					logger.log(Level.SEVERE, "IO error sending TRYING");
				}
			}
		}
	}
	...
	class TransactionTimer extends SIPStackTimerTask {
		public TransactionTimer() {
			logger.log(Level.FINEST, "TransactionTimer(): {0}", getTransactionId());
		}
	...
	protected void sendResponse(SIPResponse transactionResponse) throws IOException {
		logger.log(Level.FINEST, "sipServerTransaction::sendResponse: {0}", transactionResponse.getFirstLine());
		...
		try {
			...
			if(isReliable()) {
				...
				MessageChannel messageChannel = ((SIPTransactionStack) getSIPStack()).createRawMessageChannel(
						this.getSipProvider().getListeningPoint(hop.getTransport()).getIPAddress(), this.getPort(),
						hop);
	...
	protected SIPServerTransactionImpl(SIPTransactionStack sipStack, MessageChannel newChannelToUse) {
		...
		logger.log(Level.FINEST, "Creating Server Transaction: {0}", this.getBranchId());
	}
		...
		if(isInviteTransaction() || !isTerminated()) {
			...
			if(topViaHeader != null) {
				...
				if(messageBranch != null && this.getBranch() != null) {
					...
					// force the re-parsing only on non RFC 3261 messages
					origRequest = (SIPRequest) getRequest();

					/*
					 * This is an RFC2543-compliant message; this code is here for backwards compatibility. It is
					 * a weak check. If RequestURI, To tag, From tag, CallID, CSeq number, and top Via headers are
					 * the same, the SIPMessage matches this transaction. An exception is for a CANCEL request, which
					 * is not deemed to be part of an otherwise-matching INVITE transaction.
					 */
					...
					if(messageToTest.getCSeq().getMethod().equalsIgnoreCase(Request.CANCEL)
							&& !origRequest.getCSeq().getMethod().equalsIgnoreCase(Request.CANCEL)) {
						transactionMatches = false;
					} else if ((isResponse
							|| origRequest.getRequestURI().equals(((SIPRequest) messageToTest).getRequestURI()))
							...
							&& origRequest.getCallId().getCallId()
									.equalsIgnoreCase(messageToTest.getCallId().getCallId())
							&& origRequest.getCSeq().getSeqNumber() == messageToTest.getCSeq().getSeqNumber()
	...
	public void map() {
		...
		if(realState < 0 || realState == TransactionState.TRYING_VALUE) {
	...
	public void processRequest(SIPRequest transactionRequest, MessageChannel sourceChannel) {
		...
		logger.log(Level.FINEST, "processRequest: {0}", transactionRequest.getFirstLine());
		logger.log(Level.FINEST, "tx state: {0}", this.getRealState());
		...
		try {
			...
			if(getRealState() < 0) {
				...
				// If an invite transaction is ACK'ed while in the completed state,
			} else if (isInviteTransaction() && TransactionState.COMPLETED_VALUE == getRealState()
					&& transactionRequest.getMethod().equals(Request.ACK)) {
				this.setState(TransactionState.CONFIRMED_VALUE);
				...
				// For the purpose of testing a TI, added a property to pass it anyway
				if(sipStack.isNon2XXAckPassedToListener()) {
					// This is useful for test applications that want to see all messages.
					requestOf.processRequest(transactionRequest, encapsulatedChannel);
				} else {
					// According to RFC3261 Application should not ACK in CONFIRMED state
					logger.log(Level.FINEST, "ACK received for server TX: {0} not delivering to application!",
				...
				// If we receive a retransmission of the original request,
			} else if(transactionRequest.getMethod().equals(getMethod())) {
				if(TransactionState.PROCEEDING_VALUE == getRealState() || TransactionState.COMPLETED_VALUE == getRealState()) {
					this.semRelease();
					// Resend the last response to the client Send the message to the client
					resendLastResponseAsBytes();
				} else if(transactionRequest.getMethod().equals(Request.ACK)) {
					// This is passed up to the TU to suppress retransmission of OK
					if(requestOf != null) {
						requestOf.processRequest(transactionRequest, encapsulatedChannel);
					} else {
						this.semRelease();
				...
				logger.log(Level.FINEST, "completed processing retransmitted request: {0}, {1}, txState: {2},"
						+ " lastResponse: {3}", new Object[] {transactionRequest.getFirstLine(), this, this.getState(),
								this.lastResponseAsBytes});

				return;
			}

			// Pass message to the TU
			if (TransactionState.COMPLETED_VALUE != getRealState() && TransactionState.TERMINATED_VALUE != getRealState()
					&& requestOf != null) {
				...
				if(SIPTransactionStack.isDialogCreated(getMethod()) && getRealState() == TransactionState.TERMINATED_VALUE
						&& transactionRequest.getMethod().equals(Request.ACK) && requestOf != null) {
					...
					try {
						...
						// Transaction is already terminated just ignore the IOException.
					}
				} else {
					// none of the above? well release the lock anyhow!
					this.semRelease();
				...
				logger.log(Level.FINEST, "Dropping request {0}", getRealState());
			}
		} catch(IOException e) {
	...
	public void sendMessage(SIPMessage messageToSend) throws IOException {
		logger.log(Level.FINEST, "sipServerTransaction::sendMessage: {0}", messageToSend.getFirstLine());
		...
		try {
			try {
				...
				logger.log(Level.SEVERE, "UnexpectedException", ex);
			...
			if (!checkStateTimers(statusCode)) {
				logger.log(Level.FINEST, "checkStateTimers returned false -- not sending message");
			...
			try {
				...
				logger.log(Level.FINEST, "sendMessage : tx: {0}, getState: {1}", new Object[] {this, this.getState()});
	...
	private boolean checkStateTimers(int statusCode) {
		...
		if(getRealState() == TransactionState.TRYING_VALUE) {
			if(statusCode / 100 == 1) {
				this.setState(TransactionState.PROCEEDING_VALUE);
			} else if(200 <= statusCode && statusCode <= 699) {
				/*
				 * INVITE ST has TRYING as a Pseudo state (See issue 76). We are using the TRYING pseudo state invite
				 * Transactions to signal if the application has sent trying or not and hence this check is necessary.
				 */
				if(!isInviteTransaction()) {
					if(!isReliable() && getInternalState() != TransactionState.COMPLETED_VALUE) {
						// Linger in the completed state to catch retransmissions if the transport is not reliable.
						this.setState(TransactionState.COMPLETED_VALUE);
						// Note that Timer J is only set for Unreliable transports -- see Issue 75.

						/*
						 * From RFC 3261 Section 17.2.2 (non-invite server transaction)
						 *
						 * When the server transaction enters the "Completed" state, it MUST set Timer J
						 * to fire in 64*T1 seconds for unreliable transports, and zero seconds for
						 * reliable transports. While in the "Completed" state, the server transaction
						 * MUST pass the final response to the transport layer for retransmission
						 * whenever a retransmission of the request is received. Any other final
						 * responses passed by the TU to the server transaction MUST be discarded while
						 * in the "Completed" state. The server transaction remains in this state until
					...
					/*
					 * This is the case for INVITE server transactions. essentially, it duplicates the code in the
					 * PROCEEDING case below. There is no TRYING state for INVITE transactions in the RFC. We are using
					 * it to signal whether the application has sent a provisional response or not. Hence this is
					 * treated the same as as Proceeding.
					 */

					if(statusCode / 100 == 2) {
						/*
						 * Status code is 2xx means that the transaction transitions to TERMINATED for both Reliable
						 * as well as unreliable transports. Note that the dialog layer takes care of retransmitting
						 * 2xx final responses.
						 */

						/*
						 * RFC 3261 Section 13.3.1.4 Note, however, that the INVITE server transaction
						 * will be destroyed as soon as it receives this final response and passes it to
						 * the transport. Therefore, it is necessary to periodically pass the response
						 * directly to the transport until the ACK arrives. The 2xx response is passed
						 * to the transport with an interval that starts at T1 seconds and doubles for
						 * each retransmission until it reaches T2 seconds (T1 and T2 are defined in
						 * Section 17). Response retransmissions cease when an ACK request for the
						 * response is received. This is independent of whatever transport protocols are
						...
						if(!isReliable()) {
							 ...
							 * transport layer for transmission, and the state machine MUST enter the
							 * "Completed" state. For unreliable transports, timer G is set to fire in T1
		...
		} else if(getRealState() == TransactionState.PROCEEDING_VALUE) {
			if(isInviteTransaction()) {
				// If the response is a failure message,
				if(statusCode / 100 == 2) {
					/*
					 * Set up to catch returning ACKs The transaction lingers in the terminated state for some time to
					 * catch retransmitted INVITEs
					 */
					this.disableRetransmissionTimer();
					this.disableTimeoutTimer();
				...
				// If the transaction is not an invite transaction and this is a final response,
			} else if(200 <= statusCode && statusCode <= 699) {
		...
		} else if(TransactionState.COMPLETED_VALUE == this.getRealState()) {
			return false;
		}
	...
	public void fireRetransmissionTimer() {
		try {
			logger.log(Level.FINEST, "fireRetransmissionTimer() -- {0}, state: {1}", new Object[] {this, getState()});
	...
	/**
	 * we nullify the last response very fast to save on mem and help GC but we keep it as byte array so this method
	 * is used to re-send the last response either as a response or byte array depending on if it has been nullified
	 * 
	...
	public void resendLastResponseAsBytes() throws IOException {
		if(lastResponse != null) {
			logger.log(Level.FINEST, "resend last response: {0}", lastResponse);

			sendMessage(lastResponse);
		} else if(lastResponseAsBytes != null) {
			logger.log(Level.FINEST, "resend last response: {0}", new String(lastResponseAsBytes));
			...
			if(isReliable()) {
				if (logger.isLoggable(Level.FINEST)) {
					...
					try {
						...
						logger.log(Level.FINEST, "couldn''t reparse last response: {0}",
	...
	public void fireTimeoutTimer() {
		logger.log(Level.FINEST, "SIPServerTransaction.fireTimeoutTimer this: {0} current state: {1}, method: {2}",
				new Object[] {this, getRealState(), getMethod()});

		if(isInviteTransaction() && sipStack.removeTransactionPendingAck(this)) {
			logger.log(Level.FINEST, "Found tx pending ACK - timer H has kicked");
		}

		SIPDialog dialog = (SIPDialog) getDialog();

		if((SIPTransactionStack.isDialogCreated(getMethod()) && (TransactionState.CALLING_VALUE == this.getRealState()
				|| TransactionState.TRYING_VALUE == this.getRealState())) || (getMethod().equals(Request.BYE)
						&& dialog != null && dialog.isTerminatedOnBye())) {
			dialog.setState(SIPDialog.TERMINATED_STATE);
		}

		if(TransactionState.COMPLETED_VALUE == this.getRealState() && isInviteTransaction()) {
			raiseErrorEvent(SIPTransactionErrorEvent.TIMEOUT_ERROR);

			this.setState(TransactionState.TERMINATED_VALUE);

			sipStack.removeTransaction(this);
		} else if(TransactionState.COMPLETED_VALUE == this.getRealState() && !isInviteTransaction()) {
			this.setState(TransactionState.TERMINATED_VALUE);
		...
		} else if(TransactionState.CONFIRMED_VALUE == this.getRealState() && isInviteTransaction()) {
			/*
			 * TIMER_I should not generate a timeout exception to the application when the Invite transaction is in
			 * Confirmed state. Just transition to Terminated state.
			 */
			this.setState(TransactionState.TERMINATED_VALUE);

			sipStack.removeTransaction(this);
		} else if(!isInviteTransaction() && (TransactionState.COMPLETED_VALUE == this.getRealState()
				|| TransactionState.CONFIRMED_VALUE == this.getRealState())) {
			this.setState(TransactionState.TERMINATED_VALUE);
		} else if(isInviteTransaction() && TransactionState.TERMINATED_VALUE == this.getRealState()) {
	...
	public void sendResponse(Response response) throws SipException {
		 ...
		 * longer than specified in the request.
		 */
		...
		if (statusCode == 200 && responseMethod.equals(Request.INVITE)
		...
		try {
			 ...
			 * final response until those provisional responses are acknowledged.
			 */
			final ContentTypeHeader contentTypeHeader = ((SIPResponse) response).getContentTypeHeader();
			if (this.pendingReliableResponseAsBytes != null && this.getDialog() != null
					&& this.getInternalState() != TransactionState.TERMINATED_VALUE && statusCode / 100 == 2
					&& contentTypeHeader != null
					&& contentTypeHeader.getContentType().equalsIgnoreCase(CONTENT_TYPE_APPLICATION)
					&& contentTypeHeader.getContentSubType().equalsIgnoreCase(CONTENT_SUBTYPE_SDP)) {
				if (!interlockProvisionalResponses) {
					...
					try {
						...
						logger.log(Level.SEVERE, "Interrupted acuqiring PRACK sem");
			...
			if (dialog != null) {
				if (statusCode / 100 == 2 && SIPTransactionStack.isDialogCreated(responseMethod)) {
					if (dialog.getLocalTag() == null && sipResponse.getToTag() == null) {
						...
						logger.log(Level.FINEST, "assigning toTag : serverTransaction: {0} dialog {1}, tag: {2}",
								new Object[] {this, dialog, dialog.getLocalTag()});

						sipResponse.setToTag(dialog.getLocalTag());
					} else if (dialog.getLocalTag() != null && sipResponse.getToTag() != null
							&& !dialog.getLocalTag().equals(sipResponse.getToTag())) {
						throw new SipException("Tag mismatch dialogTag is " + dialog.getLocalTag() + " responseTag is "
			...
			if(fromTag != null) {
				...
				logger.log(Level.FINEST, "WARNING -- Null From tag in request!!");
			}
			...
			} else if (dialog == null && isInviteTransaction() && this.retransmissionAlertEnabled
					&& this.retransmissionAlertTimerTask == null && response.getStatusCode() / 100 == 2) {
			...
			if (dialog != null) {
				dialog.startRetransmitTimer(this,response);
			}
	...
	public TransactionState getState() {
		...
		if(this.isInviteTransaction() && TransactionState.TRYING_VALUE == super.getInternalState()) {
	...
	public void setState(int newState) {
		// Set this timer for connection caching of incoming connections.
		if(newState == TransactionState.TERMINATED_VALUE && this.isReliable() && (!getSIPStack().cacheServerConnections)) {
			// Set a time after which the connection is closed.
			this.collectionTime = TIMER_J;
	...
	public void startTransactionTimer() {
		if((getMethod().equalsIgnoreCase(Request.INVITE) || getMethod().equalsIgnoreCase(Request.CANCEL)
				...
				&& this.transactionTimerStarted.compareAndSet(false, true)
				&& sipStack.getTimer() != null && sipStack.getTimer().isStarted()) {
			// The timer is set to null when the Stack is shutting down.
	...
	protected void startTransactionTimerJ(long time) {
		if(this.transactionTimerStarted.compareAndSet(false, true) && sipStack.getTimer() != null
				&& sipStack.getTimer().isStarted()) {
			logger.log(Level.FINEST, "starting TransactionTimerJ(): {0} time {1}",
	...
	public void setDialog(SIPDialog sipDialog, String dialogId) {
		logger.log(Level.FINEST, "setDialog {0} dialog: {1}", new Object[] {this, sipDialog});
	...
	public void sendReliableProvisionalResponse(Response relResponse) throws SipException {
		 ...
		 * acknowledged, the UAS MAY send additional reliable provisional responses. The
		 * UAS MUST NOT send a second reliable provisional response until the first is
		 * acknowledged.
		 */
	...
	public void releaseSem() {
		if (this.pendingSubscribeTransaction != null) {
			 ...
			 * racing with the OK of the subscribe.
			 */
	...
	public void scheduleAckRemoval() {
		if(this.getMethod() == null || !this.getMethod().equals(Request.ACK)) {
			throw new IllegalStateException("Method is null[" + (getMethod() == null) + "] or method is not ACK["
					+ this.getMethod() + "]");
		}
	...
	/**
	 * cleanup the state of the stx to help GC
	 * 
	...
	public void cleanUp() {
		...
		logger.log(Level.FINEST, "removing: {0}", this);
		...
		if(isReleaseReferences()) {
			...
			if(originalRequest == null && originalRequestBytes != null) {
				try {
					...
					logger.log(Level.SEVERE, "message " + originalRequestBytes + "could not be reparsed !");
				}
			...
			cleanUpOnTimer();
			/*
			 * commented out because the application can hold on a ref to the tx after it has been removed from
			 * the stack and want to get the request or branch from it
			 */
		...
		if((!sipStack.cacheServerConnections) && isReliable() && --getMessageChannel().useCount <= 0) {
			...
			if((!sipStack.cacheServerConnections) && isReliable()) {
				...
				logger.log(Level.FINEST, "Use Count: {0}", useCount);
			}
		}
	}
	...
	protected void cleanUpOnTimer() {
		if(isReleaseReferences()) {
			logger.log(Level.FINEST, "cleanup on timer: {0}", getTransactionId());
