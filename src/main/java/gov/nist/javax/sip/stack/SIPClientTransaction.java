...
public interface SIPClientTransaction extends ClientTransactionExt, SIPTransaction, ServerResponseInterface {
	...
	/**
	 * Process a new response message through this transaction. If necessary, this
	 * message will also be passed onto the TU.
	 * 
	 * @param transactionResponse Response to process.
	 * @param sourceChannel       Channel that received this message.
	 */
	public abstract void processResponse(SIPResponse transactionResponse, MessageChannel sourceChannel,
			SIPDialog dialog);
	...
	/**
	 * Sets a timeout after which the connection is closed (provided the server does
	 * not use the connection for outgoing requests in this time period) and calls
	 * the superclass to set state.
	 */
	...
	/**
	 * Check if the From tag of the response matches the from tag of the original
	 * message. A Response with a tag mismatch should be dropped if a Dialog has
	 * been created for the original request.
	 * 
	...
	// jeand : cleanup method to clear the state of the tx once it has been removed
	// from the stack
	...
	/**
	 * will terminate a null state dialog when the transaction terminates Default:
	 * true
	 * 
	 * @param enabled
	 */
