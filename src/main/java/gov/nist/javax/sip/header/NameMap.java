...
/**
 * A mapping class that returns the SIPHeader for a given header name. Add new
 * classes to this map if you are implementing new header types if you want some
 * of the introspection based methods to work.
 * 
...
public class NameMap implements SIPHeaderNames {
	...
	protected static void putNameMap(String headerName, String className) {
		map.put(headerName.toLowerCase(), className);
	}

	public static Class<?> getClassFromName(String headerName) {
		String className = map.get(headerName.toLowerCase());
	 ...
	 * @param className  is the fully qualified class name that implements the
	 *                   header (does not have to belong to the nist-sip package).
	 *                   Use this if you want to use the introspection-based
	 *                   methods.
	 */
