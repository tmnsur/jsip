 ...
 * safely used in the interim between now and the next release. SipStackImpl
 * implements this interface.
 *
 * The following new stack initialization flags are defined (not the gov.nist
 * prefix will be dropped when the spec is updated):
 *
 * <ul>
...
public interface SipStackExt extends SipStack {
	/**
	 * Get the collection of dialogs currently in the Dialog table. This is useful
	 ...
	 *
	 * @return - the authentication helper which can be used for generating the
	 *         appropriate headers for handling authentication challenges for user
	 *         agents.
	 *
	 * @since 2.0
	 */
	...
	/**
	 * Set the address resolution interface. The address resolver allows you to
	 * register custom lookup schemes ( for example DNS SRV lookup ) that are not
	 * directly supported by the JDK.
	 *
	 ...
	 *                     random port).
	 *
	 * @return the local SocketAddress that this handler would use when connecting
