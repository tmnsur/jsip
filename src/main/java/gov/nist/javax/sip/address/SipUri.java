...
public class SipUri extends GenericURI implements javax.sip.address.SipURI, SipURIExt {
	...
	private static final Logger logger = Logger.getLogger(SipUri.class.getName());
	 ...
	 *
	 *         JvB: Updated to define equality in terms of API methods, according to
	 *         the rules in RFC3261 section 19.1.4
	 *
	 *         Jean Deruelle: Updated to define equality of API methods, according
	 *         to the rules in RFC3261 section 19.1.4 convert potential ie : %HEX
	 *         HEX encoding parts of the URI before comparing them transport param
	 *         added in comparison header equality enforced in comparison
	 *
	 */
	@Override
	public boolean equals(Object that) {
		...
		if (that instanceof SipURI) {
			...
			if (a.getUserPassword() != null
					&& !UriDecoder.decode(a.getUserPassword()).equals(UriDecoder.decode(b.getUserPassword())))
				return false;
			...
			if (a.getHeaderNames().hasNext() && b.getHeaderNames().hasNext()) {
				...
				try {
					...
					logger.log(Level.SEVERE, "Cannot get the header factory to parse the header of the sip uris to compare", e);
				...
				for(Iterator i = a.getHeaderNames(); i.hasNext();) {
					...
					try {
						...
						// those present in both must match according to the equals method of the
						// corresponding header
						if (!header1.equals(header2))
							return false;
					} catch (ParseException e) {
						logger.log(Level.SEVERE, e, () -> MessageFormat.format("Cannot parse one of the header"
								+ " of the sip uris to compare {0} {1}", a, b));
	...
	/**
	 * Returns the stucture corresponding to the telephone number provided that the
	 * user is a telephone subscriber.
	 * 
	 * @return TelephoneNumber part of the url (only makes sense when user = phone
	 *         is specified)
	 */
	 ...
	 * Internet telephony gateway, a telephone-subscriber field MAY be used instead
	 * of a user field. The telephone-subscriber field uses the notation of RFC 2806
	 * [19]. Any characters of the un-escaped "telephone-subscriber" that are not
	 * either in the set "unreserved" or "user-unreserved" MUST be escaped. The set
	 * of characters not reserved in the RFC 2806 description of
	 * telephone-subscriber contains a number of characters in various syntax
	 * elements that need to be escaped when used in SIP URLs, for example quotation
	 * marks (%22), hash (%23), colon (%3a), at-sign (%40) and the "unwise"
	 * characters, i.e., punctuation of %5b and above.
	 *
	 * The telephone number is a special case of a user name and cannot be
	 * distinguished by a BNF. Thus, a URL parameter, user, is added to distinguish
	 * telephone numbers from user names.
	 *
	 ...
	 * interpret the pre-@ part as a telephone number if local restrictions on the
	 * 
	...
	/**
	 * Sets the user of SipURI. The identifier of a particular resource at the host
	 * being addressed. The user and the user password including the "at" sign make
	 * up the user-info.
	 *
	...
	/**
	 * Set a given URI parameter. Note - parameter must be properly encoded before
	...
	/**
	 * Returns an Iterator over the names (Strings) of all headers present in this
	 * SipURI.
	 *
	...
	/**
	 * Returns the value of the <code>lr</code> parameter, or null if this is not
	 * set. This is equivalent to getParameter("lr").
	 *
	 ...
	 * set. This is equivalent to getParameter("maddr").
	 *
	...
	/**
	 * Returns the value of the <code>method</code> parameter, or null if this is
	 * not set. This is equivalent to getParameter("method").
	 *
	...
	/**
	 * Returns true if this SipURI is secure i.e. if this SipURI represents a sips
	 ...
	 * parameter will be used in the URIs proxies place in the Record-Route header
	 * field values, and may appear in the URIs in a pre-existing route set.
	 */
	...
	/**
	 * Sets the value of the <code>maddr</code> parameter of this SipURI. The maddr
	 * parameter indicates the server address to be contacted for this user,
	 * overriding any address derived from the host field. This is equivalent to
	...
	/**
	 * Sets the value of the <code>method</code> parameter. This specifies which SIP
	 * method to use in requests directed at this URI. This is equivalent to
	...
	/**
	 * Sets the scheme of this URI to sip or sips depending on whether the argument
	 * is true or false. The default value is false.
	 *
	...
	/**
	 * Sets the value of the <code>ttl</code> parameter. The ttl parameter specifies
	 * the time-to-live value when packets are sent using UDP multicast. This is
	 * equivalent to setParameter("ttl", ttl).
	 *
	...
	/**
	 * Sets the value of the "transport" parameter. This parameter specifies which
	 * transport protocol to use for sending requests and responses to this entity.
	 * The following values are defined: "udp", "tcp", "sctp", "tls", "ws", "wss"
	 * but other values may be used also. This method is equivalent to
	...
	/**
	 * Returns whether the the <code>lr</code> parameter is set. This is equivalent
	 * to hasParameter("lr"). This interface has no getLrParam as RFC3261 does not
	 * specify any values for the "lr" paramater.
	 *
