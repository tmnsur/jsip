...
class CredentialsCache {
	 ...
	 * user for a number of domains. The Hashtable value of the mapping is a mapping
	 * of user names to AuthorizationHeader list for that proxy domain.
	 */
	 ...
	 *
	 * @param callid        the id of the call that the <tt>authorization</tt>
	 *                      header belongs to.
	...
	void cacheAuthorizationHeader(String callId, AuthorizationHeader authorization, int cacheTime) {
		...
		if(cacheTime != -1) {
			this.timer.schedule(timeoutTask, cacheTime * 1000L);
		}
	}

	/**
	 * Returns an authorization header cached for the specified call id and null if
	 * no authorization header has been previously cached for this call.
	 *
	 * @param callid the call id that we'd like to retrive a cached authorization
	 *               header for.
	 *
	 * @return authorization header corresponding to that user for the given proxy
	 *         domain.
	 */
