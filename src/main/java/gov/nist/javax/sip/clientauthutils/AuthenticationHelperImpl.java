...
public class AuthenticationHelperImpl implements AuthenticationHelper {
	private static final Logger logger = Logger.getLogger(AuthenticationHelperImpl.class.getName());
	...
	/**
	 * Default constructor for the security manager. There is one Account manager.
	 * There is one SipSecurity manager for every user name,
	 *
	 ...
	 *
	 * @see
	 * gov.nist.javax.sip.clientauthutils.AuthenticationHelper#handleChallenge(javax
	 * .sip.message.Response, javax.sip.ClientTransaction, javax.sip.SipProvider)
	 */
	public ClientTransaction handleChallenge(Response challenge, ClientTransaction challengedTransaction,
	 ...
	 */
	public ClientTransaction handleChallenge(Response challenge, ClientTransaction challengedTransaction,
			SipProvider transactionCreator, int cacheTime, boolean looseRouting) throws SipException {
		try {
			logger.log(Level.FINEST, "handleChallenge: {0}", challenge);

			SIPRequest challengedRequest = ((SIPRequest) challengedTransaction.getRequest());
			Request reoriginatedRequest = null;

			/*
			 * If the challenged request is part of a Dialog and the Dialog is confirmed the
			 * re-originated request should be generated as an in-Dialog request.
			 */
			if(challengedRequest.getToTag() != null || challengedTransaction.getDialog() == null
				...
				/*
				 * Re-originate the request by consulting the dialog. In particular the route
				 * set could change between the original request and the in-dialog challenge.
				 */
			...
			/*
			 * Resolve this to the next hop based on the previous lookup. If we are not
			 * using lose routing (RFC2543) then just attach hop as a maddr param.
			 */
			...
			while(authHeaders.hasNext()) {
				...
				if(this.accountManager instanceof SecureAccountManager) {
					UserCredentialHash credHash = ((SecureAccountManager) this.accountManager)
					...
					authorization = this.getAuthorization(reoriginatedRequest.getMethod(), uri.toString(),
							(reoriginatedRequest.getContent() == null) ? ""
									: new String(reoriginatedRequest.getRawContent()),
							authHeader, credHash);
				} else {
					UserCredentials userCreds = ((AccountManager) this.accountManager)
					...
					authorization = this.getAuthorization(reoriginatedRequest.getMethod(),
				...
				logger.log(Level.FINEST, "Created authorization header: {0}", authorization);
			...
			logger.log(Level.FINEST, "Returning authorization transaction: {0}", retryTran);
	...
	private AuthorizationHeader getAuthorization(String method, String uri, String requestBody,
		...
		response = MessageDigestAlgorithm.calculateResponse(authHeader.getAlgorithm(), userCredentials.getUserName(),
				authHeader.getRealm(), userCredentials.getPassword(), authHeader.getNonce(), ncValue, cnonce, method,
				uri, requestBody, qop);
		...
		try {
			...
			if (qop != null) {
				...
				authorization.setNonceCount(Integer.parseInt(ncValue));
			}

			authorization.setResponse(response);

		} catch(ParseException ex) {
			throw new IllegalStateException("Failed to create an authorization header!");
		}
	...
	private AuthorizationHeader getAuthorization(String method, String uri, String requestBody,
		...
		response = MessageDigestAlgorithm.calculateResponse(authHeader.getAlgorithm(),
				userCredentials.getHashUserDomainPassword(), authHeader.getNonce(), ncValue, cnonce, method, uri,
				requestBody, qop);
	...
	/**
	 * Removes all via headers from <tt>request</tt> and replaces them with a new
	 * one, equal to the one that was top most.
	 *
	 ...
	 * (non-Javadoc)
	 *
	 * @see gov.nist.javax.sip.clientauthutils.AuthenticationHelper#
	 * attachAuthenticationHeaders(javax.sip.message.Request)
	 */
	public void setAuthenticationHeaders(Request request) {
		...
		if(authHeaders == null) {
			logger.log(Level.FINEST, "Could not find authentication headers for {0}", callId);
	 ...
	 *
	 * @see gov.nist.javax.sip.clientauthutils.AuthenticationHelper#
	 * removeCachedAuthenticationHeaders(java.lang.String)
	 */
